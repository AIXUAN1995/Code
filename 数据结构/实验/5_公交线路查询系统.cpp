...
思路：
1.定义边结构体EdgeNode和站点结构体Site，每个站点与能直接到达的边用链表储存，
所有站点存放在足够大的数组（数组的个数为能容纳下所有站点，并能容纳插入的站点）中。
2.建立无向图(在输入的站点的同时建立路线)。
（1）输入的时候确定路线的起始站点，定义指针指向起始站点。
（2）插入后续节点，以邻接多重表的形式储存站点
（3）建立后续线路的时候，重复（1），线路上的站点可能已经存在，需要遍历图中的站点，如果已经存在怎不在建立新站点，
只需修改原有站点中的信息即可。
（4）插入新站点的时候需要提前确定插入的站点所在的路线，以及前面的站点或后面的站点（这个站点可能同时属于多条路线，
需要用循环判断是否要将站点插入多条路线）
（5）删除站点的时候只需将该站点所关联的前面的站点直接指向后面的站点
（6）删除路线的时候只需依次删除边结点（不删除站点）
...
struct route_count						//用来记录从出发点到目的地过程中出现过的公交线路
{
	string route_name;					//记录路线名称
	int count;							//记录该路线出现的次数
	int script;							//记录该路线第一次出现的站点
};
//函数声明：
...
void ShortesPath(MGragh *m, int P[MAXSITE], float D[MAXSITE], int script);
...
{//script为要删除的站点在图中的脚标
	...
	//for (int i = 0; i < *n; i++)//将与该站点关联的边从邻接矩阵中删除
	//{
	//	if (m->edges[script][i] != INFINITY)
	//	{
	//		m->edges[script][i] = m->edges[i][script] = INFINITY;
	//	}
	//}
...
//最短路径
void ShortesPath(MGragh *m, int P[MAXSITE], double D[MAXSITE], int script1, int script2)
{
	/*
	设原点为顶点0，求到其余顶点的最短路径
	...
	*/
	...
	int change = -1;									//换乘的站点
	int change_route1;							//换乘后的路线在换乘站点存放路线数组中的位置
	...
	for (int i = 0; i < m->sites[script2].top; i++)
	{
		...
		while (script != -1)
		{
			...
			{
				if (m->sites[script2].route[i] == m->sites[script].route[j])
				{
					flag = 1;
					change = script;
					change_route1 = j;
			...
			if (flag == 0)
			{
				route2 = m->sites[script2].route[i];
				break;
			}
			script = P[script];
		}
		if (change == script || change == -1)
			continue;
...
int main()
{
	...
	while (1)
	{
			...
			else if (ch_inquire == '3')								//查询两个站点之间的公交线路
			{
				...
				if (flag == 2)//起始站点和目的地都存在
				{
					...
					if (flag_arrive == 0)//没有直达路线
					{
						...
						ShortesPath(&m, P, D, script1, script2);
					}
				}
				cout << endl;
				system("pause");
		...
		else if (ch_primary != '1' && ch_primary != '2' && ch_primary != '3')
		{
			cout << "没有您选择的操作！" << endl;
			system("pause");
		}
