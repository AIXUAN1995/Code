...
//函数声明
int getCar(STOPPING *stop, string license_plate);				//获取要出来的汽车在停车场的位置
void display(STOPPING *stop, BUFFER *buffer, PAVEMENT *pave);	//查看停车场停车状态的查询模块
...
void come(STOPPING *stop, CAR *car, BUFFER *buffer, PAVEMENT *pave)//汽车进入停车位模块管理
{
exit:
...
void leave(STOPPING *stop, BUFFER *buffer, PAVEMENT *pave)//汽车离开停车场管理模块
{
	cout << "输入要离开车辆的车牌：" << endl;
	string license_plate;
	cin >> license_plate;
	int n = getCar(stop, license_plate);		//若停车场中有汽车返回位置，否则返回-1
	if (n == -1)
	{
...
void display(STOPPING *stop, BUFFER *buffer, PAVEMENT *pave)//查看停车场停车状态的查询模块
{
	...
	else
	{
		...
		for (int i = 0; i <= stop->top; i++)
			cout << i + 1 << "号车位------" << stop->STOP[i].license_plate << endl;
		if (pave->rear > pave->front)
		{
			cout << "便道中的车辆信息如下：" << endl;
			for (int i = pave->front + 1, j = 1; i <= pave->rear; i++)
				cout << j++ << "号位置------" << pave->PAVE[i].license_plate << endl;
		}
